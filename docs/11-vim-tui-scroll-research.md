...
`smoothScrollback` сравнивает точные сигнатуры строк последнего кадра и нового
кадра и анимирует только один однозначный полноширинный сдвиг. В normal buffer
...
* Новый output заменяет активную анимацию последним logical target; очередь кадров
  не создаётся. Длительность ограничена 100–240 ms.
