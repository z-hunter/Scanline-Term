...
| Существующая плавность | `useTerminal.ts`: normal-buffer user scroll и output autoscroll вызывают SVS `TerminalRenderer.beginScroll(fromViewportY,toViewportY)`. После `onWriteParsed` SVS сравнивает `rowSignatures` и при единственном вертикальном кандидате создаёт тот же `ScrollTransition` с row clip: в alternate buffer всегда, в normal buffer — только без изменения `baseY`/`viewportY` у нижней границы. |
