...
| Существующая плавность | `useTerminal.ts`: normal-buffer user scroll и output autoscroll вызывают SVS `beginBufferScroll(fromViewportY,toViewportY)`. Локальный `terminal-scroll-heuristic.ts` после `onWriteParsed` сравнивает snapshots и при единственном вертикальном кандидате вызывает SVS `beginRegionScroll()`: в alternate buffer всегда, в normal buffer — только без изменения `baseY`/`viewportY` у нижней границы. |
...
* Новый output расширяет совместимый переход: сохраняется один чистый source
  snapshot, а меняется только абсолютная цель и тайминг с сохранением текущего
  pixel offset. При отставании более шести строк fast-режим остаётся включённым
  до завершения активного перехода; несовместимый вид/регион/направление или
  сокращение цели захватывает текущий visual frame и начинает новый переход.
  Обычный переход ограничивает скорость примерно 240 px/s, fast длится 100–240 ms.
