...
Each xterm session preserves 10,000 scrollback lines. User-driven normal-buffer scrolling (wheel or the existing Menu+PageUp/PageDown and Menu+J/K shortcuts) publishes the active session's viewport/base/row counts to `ui/ScrollbackScrollbar.tsx`. The indicator is not activated by ordinary output, alternate-screen applications, or mouse-tracking wheel events. Its thumb uses pointer capture and calls xterm `scrollToLine()` while dragging; when smooth terminal scrolling is enabled, ordinary wheel/slow-drag/output movement uses the smooth speed (240 px/s), while backlog beyond six rows latches the active transition into the fast 100–240 ms mode. Fast drags switch to the immediate path before animation starts. New compatible targets extend the same clean source snapshot without restarting the transition; direction changes, shortening targets, kind changes, or region changes capture the current visual frame and start a new transition. Its nested `Heuristic TUI scrolling` switch independently enables a conservative frame-diff heuristic for alternate-buffer TUI frames and for normal-buffer TUI updates that leave `baseY` and `viewportY` unchanged while the user is at the bottom. The SVS terminal renderer compares row signatures and animates only one unambiguous full-width vertical shift with at least four matching rows (three containing text); up to two rows may differ only in presentation (colours or text decoration) while their characters, cell widths and visibility remain identical, to tolerate editor match highlighting. Normal scrollback growth stays on its existing path. Any ambiguity, changed grid dimensions, selection, resize, or non-output repaint is shown immediately. It fades after two seconds of inactivity. The renderer also keeps a bounded smooth-scroll diagnostic ring buffer and exposes a `Copy log` action in Settings → UI; collection is disabled in normal builds and can be enabled temporarily in the SVS renderer when investigating a missed or partial candidate.
...
`scanline-term [options] [target] [arguments...]` starts the first tab. `-T` and `-P <path>` are launcher options before the target; `--` can explicitly end launcher-option parsing. An existing local file, including a `file:///…` URI, opens in the embedded browser; an existing directory opens the default shell there; other file paths or executable names run as commands, with the following arguments passed unchanged. A second `scanline-term -T [target] [arguments...]` is routed to the existing application and opens the requested browser file or terminal session in a new tab.
...
| **Menu+Shift+/** | Open terminal-buffer search in reverse direction | `terminal/useTerminal.ts` keyboard handler |
...
When search is open, the DOM search field owns text input and does not forward it to ConPTY. Matches are highlighted in `TerminalRenderer` before the CRT pipeline; the compact search field is a DOM overlay above the output canvas. `Menu+/` labels the field `Find` and makes Enter/`n` move down; `Menu+Shift+/` labels it `Find back` and makes Enter/`n` move up. `N` reverses the active direction. Enter moves focus back to the terminal canvas; invoking either search shortcut again focuses the field. Normal-buffer searches include scrollback, while alternate-buffer searches are limited to the current screen.
...
**Dirty-driving:** `onWriteParsed()` and cursor movement mark the terminal for comparison, not an automatic full repaint. The renderer snapshots every row's visible cell state and redraws only changed rows plus the old/new cursor row. A scroll, source resize, font/profile change, or selection change invalidates the whole source canvas. The source canvas is cursor-free; the current cursor and tab images are composited after any scroll transition so hit-testing and logical input always follow the new grid. Normal-buffer scrollback and the alternate-buffer heuristic share one 100–240 ms pixel transition; a new output frame replaces an active transition rather than queueing it.
...
The persisted `globalHotkeyEnabled` setting invokes `set_global_hotkey_enabled`; `slideFromTop` selects the optional native slide animation. When enabled, the Rust side registers `Win+~` (`SUPER+Backquote`) with Windows. Pressing it hides Scanline Term only when it is already focused; otherwise it shows and focuses the main window, optionally animating it down from above the monitor and back up when hidden. A registration failure, such as an OS-reserved or already claimed shortcut, is reported to the UI and resets the setting to disabled.

### Font Size and Cell Measurement

`fontCellSize(fontSize, fontFamily, context, cellWidthAdjustment, cellHeightAdjustment)` in the SVS terminal renderer:
