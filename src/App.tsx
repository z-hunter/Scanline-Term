...
export default function App() {
  ...
  return <main style={{ '--settings-scale': String(stored.settingsScale) } as CSSProperties} className={`app-shell${settingsVisible ? '' : ' settings-hidden'}`}><section className="display-panel" aria-label="CRT display"><div ref={workspaceRef} style={{ '--tab-space': '36px' } as CSSProperties} className={`terminal-workspace terminal-workspace-${stored.tabPlacement}`}>{isTauri() && <TerminalTabs panelRef={tabsRef} tabs={terminal.tabs} activeId={terminal.activeSessionId} placement={stored.tabPlacement} hideTabList={stored.hideTabsWhenSingleSession && terminal.tabs.length <= 1} onSelect={terminal.selectSession} onClose={terminal.closeSession} onNew={() => terminal.openSession()} onToggleSettings={toggleSettings} />}<div id="terminal-display" className={`screen-frame${physicalWindow ? ' physical-window' : ''}${stored.crt.showBezel ? '' : ' bezel-hidden'}`} style={screenStyle}><canvas ref={outputRef} className="output-canvas" data-testid="output-canvas" tabIndex={terminal.live ? 0 : -1} aria-label={terminal.live ? 'Windows console' : 'CRT display'} {...terminal.canvasProps} /><span className="frame-status">{terminal.size.cols} × {terminal.size.rows}</span></div></div>{error && <p className="error" role="alert">{error}</p>}</section>{settingsVisible && <SettingsPanel stored={stored} setStored={setStored} monospaceFonts={terminal.fonts} terminalSize={terminal.size} fps={fps} renderStats={renderStats} onReset={reset} />}</main>;
}
