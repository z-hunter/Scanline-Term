...
export type TerminalSearchState = { open: boolean; query: string; matches: TerminalSearchMatch[]; activeIndex: number; buffer: 'normal' | 'alternate' | null };
...
export function useTerminal({ defaultPreset, ready = true, settings, resolution, defaultShell = '', smoothScrollback = false, smoothTuiScrolling = true, shells = [], onError, onToggleSettings, onToggleAi, onTerminalTabTransition }: { defaultPreset?: PresetSettings; ready?: boolean; settings?: CRTSettings; resolution?: Resolution; defaultShell?: string; smoothScrollback?: boolean; smoothTuiScrolling?: boolean; shells?: ShellInfo[]; onError: (message: string) => void; onToggleSettings: () => void; onToggleAi?: () => void; onTerminalTabTransition?: () => void }) {
  ...
  const [activePresetState, setActivePresetState] = useState<TabPresetState | null>(null); const [scrollback, setScrollback] = useState<ScrollbackScrollbarState | null>(null); const [search, setSearch] = useState<TerminalSearchState>({ open: false, query: '', matches: [], activeIndex: -1, buffer: null });
  ...
  const clearSearch = useCallback(() => {
    searchRef.current = { open: false, query: '', matches: [], activeIndex: -1, buffer: null };
    setSearch(searchRef.current);
    renderer.current?.setSearchMatches([]);
  }, []);
  const applySearch = useCallback((query: string, activeIndex = 0) => {
    ...
    const next = { open: true, query, matches: snapshot.matches, activeIndex: nextIndex, buffer: snapshot.buffer } satisfies TerminalSearchState;
    ...
    if (match && snapshot.buffer === 'normal') {
      ...
      if (to !== from && smoothScrollbackRef.current) renderer.current?.beginScroll(from, to);
  ...
  const moveSearch = useCallback((direction: 1 | -1) => {
    ...
    applySearch(current.query, nextSearchIndex(current.activeIndex, current.matches.length, direction));
  }, [applySearch]);
  ...
  }, [applySearch, onError, publishScrollback, refreshTabColor, selectSession, updateTab]);
